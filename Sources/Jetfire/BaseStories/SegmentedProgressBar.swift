...
class SegmentedProgressBar: UIView {
	...
	func startAnimation(from index: Int = 0) {
		self.layoutSubviews()
		self.fillSegments(till: index)
		self.animate(animationIndex: index)
		self.delegate?.segmentedProgressBarChangedIndex(index: index)
    }

	private func fillSegments(till index: Int) {
		for i in 0..<index {
			if let segment = self.segments.safeObject(at: i) {
				segment.topSegmentView.frame.size.width = segment.bottomSegmentView.frame.width
			}
		}
	}

    private func animate(animationIndex: Int) {
